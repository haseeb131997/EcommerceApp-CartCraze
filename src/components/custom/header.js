...
const Header = ({title,leftIcon, rightIcon, onClickLeftIcon, onClickRightIcon, isCart}) => {
  ...
  return (
    <View style={styles.header}>
      ...
      {!isCart && <View></View>}
      {isCart && <TouchableOpacity style={styles.btn} onPress={()=>{navigation.navigate('CartScreen')}}>
        <Image source={rightIcon} style={[styles.icon, {width:40, height:40}]} />
        <View style={{width:20, height:20, borderRadius:10, backgroundColor:'#ffff', position:'absolute', right:0, top:0, alignItems:'center', justifyContent:'center'}}>
